...
<div class="container">
    <div class="pannel">
        ...
        <div class="logout" style="float:right; font-size:20px;margin-right:16px;padding-top:15px;"> 
            <a href="edit-users.php" style="color:rgb(85, 83, 83);margin-left:5px;">EDIT USERS</a>
        </div>
    </div>
    <div>
        <?php
            if (isset($_POST['new'])) {
                ...
                $category = getClean($_POST['category']);
                if (!empty($_POST['photo']) && !empty($_POST['name']) && !empty($_POST['price']) && !empty($_POST['category'])) {
                    $query = "INSERT INTO `products` (`name`, `price`, `photo`, `cat_id`) VALUES ('$name', '$price', '$photo', '$category')";
